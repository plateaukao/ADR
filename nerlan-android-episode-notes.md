...
Course episodes on Channel+ are often titled just "EP12", so the episode lists give no clue what a lesson actually covers. The iOS app gained per-episode user notes yesterday (commit `fd56dfb`); this change brings the same feature to Android (`4ce20a9`). Long-pressing any episode row — in the program episode list, Downloads, Favorites, a podcast's episode list, or the AI tab — opens a small dialog to write a note, and the note then appears under that episode's metadata in every list.
