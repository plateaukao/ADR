...
- **Reconnect-on-failure is a feature, not a bug.** `adb kill-server` followed by `adb start-server` cleanly cycles: domains disappear within about 1 s of the kill (our error handler clears `devices`, `.onChange` fires, reconcile removes domains), and reappear within about 3-5 s of the restart (back-off + reconnect + first stream yield). No app restart needed.
