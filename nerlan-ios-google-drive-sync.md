...
2. **Transport — `URLSession` against Drive REST v3.** Direct HTTPS to `googleapis.com/drive/v3`, scope `drive.appdata` only. A 1:1 port of Android's OkHttp calls (`listFiles`, multipart `upsert` for create, `PATCH uploadType=media` for update). ~150 lines.
